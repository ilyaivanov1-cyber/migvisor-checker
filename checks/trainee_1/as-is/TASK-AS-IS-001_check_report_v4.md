...
checked_at: 2026-09-01T17:00:00Z
...
**Improvement items:**
- [ ] Add `Owner / Contact` column — team name, role, email, or "TBD" per row. Currently completely absent (0 pts for this attribute).
...
**Coverage (7/7):** Complete pipeline documented end-to-end: OLTP sources → extract procedures → staging → SCD2 key resolution → fact load → ETL metadata → analytics layer → BI reports. Five subsections: key columns/metrics table, Mermaid flow diagram, column-level lineage table, 14-step transformation table, downstream dependencies table.
...
**Technical accuracy (3/3):** Top-down flow direction correct. OLTP-direct report (`dae – global`) correctly distinguished as bypassing the DW ETL pipeline.

**Issues/gaps flagged (2/2):** Undocumented 1.05 business factor flagged, OLTP-direct bypass noted as a lineage gap, Azure Blob ad-hoc exclusion from daily ETL stated, no Power BI consumer captured for `v_OrderToSupplyAnalytics` and `v_OrderToYearAnalytics`.

**Structure (1/1):** Five clearly delineated subsections (§4.1–§4.5) with consistent table and diagram formats.
...
**Coverage (7/7):** All seven calculations documented — Total Excluding Tax (Sale + Order variants), Tax Amount, Total Including Tax, Total Dry Items, Total Chiller Items, ProfitMarginWithFactor, GetTotalQuantitySold UDF. Each entry includes business purpose, formula, input table/column inventory, SQL code, step-by-step walkthrough, and threshold table.

**Specificity (4/4):** Exact SQL expressions shown, Sale vs. Order path differences explicitly distinguished per calculation, specific CTE names and logic in `CustomerSalesSummary` documented, UDF parameter names noted.

**Technical accuracy (3/3):** All calculations reference the correct source layer (OLTP for extracts, DW facts for analytics). Sale and Order monetary calculation differences (pre-stored vs. re-derived) accurately described.

**Issues/gaps flagged (2/2):** Undocumented 1.05 uplift factor explicitly flagged as a business-rule gap; NULL return without COALESCE guard in both UDF variants noted; ±0.01 rounding discrepancy from independently rounded Order components documented.

**Structure (1/1):** Seven subsections (§5.1–§5.7) each with identical internal structure (purpose → formula → inputs → SQL → steps → thresholds).
...
**Coverage (7/7):** 19 source objects across three platforms (SQL Server `wideworldimporters`, SQL Server `wideworldimportersdw` control objects, Azure Blob via PolyBase) + 15 output objects. Platform, system, schema, object type, description, and key fields populated for each entry.

**Specificity (4/4):** Connection methods named per source (SSIS, PolyBase, OLTP-direct view), temporal table variants noted (`_Archive`), specific column names listed as key fields, Azure Blob URL and format documented.

**Technical accuracy (3/3):** Input sources (§6.1) and output targets (§6.2) correctly separated. No source/target confusion.

**Issues/gaps flagged (2/2):** Azure Blob PolyBase excluded from daily ETL and marked ad-hoc. OLTP-direct `WebApi.SalesOrders` bypass documented. `Sale_Staging` lacks LineageKey (asymmetric vs. `Order_Staging`) noted in §6.2.

**Structure (1/1):** §6.1 Input Source Tables + §6.2 Output Tables. Consistent column layout across both tables.
...
- Section 3 uses a Mermaid `erDiagram` — accepted as full substitute for a relationships table; cardinality correctly expressed (including nullable `PickerKey`).
...
- Section 5 "no-calculations" approach note: not applicable here — calculations are present and documented.
...
1. **Section 2 — Coverage: owner/contact and frequency/SLA absent — 4 pts recoverable:** Adding both columns to the consumer table would bring Section 2 coverage from 4/6 to 6/6.
2. **Section 2 — Issues/gaps flagged — 2 pts recoverable:** Add a note flagging at least one known consumer-side uncertainty (missing SLA, unknown owner, potential zombie consumer).
