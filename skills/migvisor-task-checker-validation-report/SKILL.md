...
```
/task-checker-validation-report [participant_file] [reference_file] [--product <name>] [trainee=<name>]
```
...
- `/task-checker-validation-report validation-report.md reference/validation-report.md` — both supplied
- `/task-checker-validation-report --product Sales_Orders` — both files auto-resolved; product name overridden
- `/task-checker-validation-report trainee=alice` — auto-resolve participant from trainees/alice/
- `/task-checker-validation-report` — everything auto-resolved (single trainee folder)
...
| | Path |
|---|---|
| Trainee file | `{trainee_workspace}/products/{product}/current/codebase/validation-report.md` |
| Reference file | `{reference_workspace}/module_5/reports/validation/report.md` |
| Trainee name | folder name of `{trainee_workspace}` |
| Output dir | `{checks_dir}/{trainee_name}/validation/` |
...
1. If `trainee=<name>` flag is provided: look for `trainees/<name>/validation-report.md`. If not found, abort: *"No participant file at trainees/<name>/validation-report.md. Verify the trainee name and file."*
2. Otherwise: scan `trainees/` for immediate subdirectories.
   - Exactly 1 found: use `trainees/<that-name>/validation-report.md`; record `<that-name>` as the trainee name.
...
3. Fallback: if `trainees/` does not exist, search workspace root for `validation-report.md`.

Record the resolved trainee name from the participant file path for output path construction in Step 8.

**1B — Reference file**

If a path was supplied as argument 2, use it. Otherwise look for `reference/validation-report.md`. If found: use it. If not found: abort with *"No reference file found at reference/validation-report.md. Supply an explicit reference path."*
...
2. Base name: `checks/<trainee_name>/validation/TASK-VAL-001_check_report.md`
...
Create the `checks/<trainee_name>/validation/` directory if it does not exist.
...
Written to `checks/<trainee_name>/validation/TASK-VAL-001_check_report.md` (or `_v2`, `_v3` … if prior reports exist):
