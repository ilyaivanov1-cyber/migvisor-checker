...
If a path was supplied as argument 1, use it. Otherwise prompt: *"Provide the path to your submission file, e.g. `/task-checker-as-is my-file.md`"*. Do not guess from filename patterns — require an explicit path when not supplied.

**1B — Reference file**

If a path was supplied as argument 2, use it. Otherwise search the workspace for files matching (in priority order):
1. `as-is-reference.md`
2. `reference.md`
3. `as-is.md` (only if it differs from the resolved participant file)
...
1. Base name: `TASK-AS-IS-001_check_report.md`
...
Written to `checks/TASK-AS-IS-001_check_report.md` (or `_v2`, `_v3` … if prior reports exist):
