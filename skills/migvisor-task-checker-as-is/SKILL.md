...
| | Path |
|---|---|
| Trainee file | `{trainee_workspace}/products/{product}/current/specifications/as-is.md` |
| Reference file | `{reference_workspace}/module_2/1 as-is.md` |
| Trainee name | basename of `{trainee_workspace}` (e.g. `Inventory_Stock_Project`) |
| Output dir | `{checks_dir}/{trainee_name}/as-is/` |
...
**Lineage section** — the reference traces 11 discrete ETL steps. A submission that lists only a high-level "extract → load" flow without naming individual staging, transformation, and watermark steps scores < 50% on Coverage. The SSIS truncation bug (staging table was truncated instead of purchase staging) is a distinguishing fact that well-researched submissions include. Step 2 is the bug step: `DELETE FROM Integration.Order_Staging` (wrong table) — staging is never correctly truncated. Step 11 (watermark advance) must happen AFTER a successful MERGE; advancing it before exposes a silent data loss risk if the MERGE fails.

**Calculations section** — the SCD-2 surrogate-key resolution algorithm has 3 critical subtleties that distinguish a thorough submission: (1) boundary semantics are `> Valid From` (exclusive lower bound) and `<= Valid To` (inclusive upper bound); (2) tie-breaker is `ORDER BY Valid From ASC` (earliest match, not DESC); (3) fallback is `COALESCE(..., 0)` — key 0 is a sentinel that must be pre-seeded in each dimension table. A submission that uses `>=` instead of `>`, or DESC instead of ASC, or NULL instead of 0 is technically inaccurate on SCD-2 resolution. The date_key derivation is `CAST(OrderDate AS date)` in the extract step — the only derived column at extract time; all other columns are pass-throughs.

**Model section** — the staging table has 14 columns vs. the fact table's 11. The 3 extra staging columns (`WWISupplierID`, `WWIStockItemID`, `LastModifiedWhen`) are used for SCD-2 key resolution and dropped after the MERGE. `dimension.stock item` has 19 columns including `Photo varbinary(max)`. `dimension.date` has both a `Date` (DATE) column and a `DateKey` (INT) column; the fact table uses `DateKey` as its FK.

**Sources / Consumers section** — the reference documents 8 input source objects (5 OLTP tables + 3 DW control tables) and 7 output objects. Both BI reports use direct table reads (no views). Consumer 1 (`wwidw purchase and sale per stockitem dynamic`) has high migration risk: cross-domain join with `fact.sale` forces coordinated cutover with Sales_Orders product.
