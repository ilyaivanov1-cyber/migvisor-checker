...
| | Path |
|---|---|
| Trainee file | `{trainee_workspace}/products/{product}/current/codebase/docs/runbook.md` |
| Reference file | `{reference_workspace}/module_5/codebase/docs/pipeline_runbook.md` |
| Trainee name | folder name of `{trainee_workspace}` |
| Output dir | `{checks_dir}/{trainee_name}/pipeline-runbook/` |
...
**Cross-file DQ column name check (MANDATORY)** — before scoring the DQ Investigation section, read the participant's `data-dictionary.md` at `{trainee_workspace}/products/{product}/current/codebase/docs/data-dictionary.md`. Extract the actual column names of the `dq_rejections` table. Then check every SQL query in the DQ Investigation section against those column names. If the SQL uses column names that do not exist in the data dictionary (e.g., `assertion_name` when the actual column is `rule_id`, or `violation_type` when the actual column is `violation_column`), mark each wrong column as a [DEFECT] and subtract from the SQL Queries criterion score proportionally (−5 pts per wrong column name pair, max −10 pts for the DQ Investigation section). If `data-dictionary.md` is not present, fall back to the reference data dictionary and apply the same check.
