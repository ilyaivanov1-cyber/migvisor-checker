...
```bash
databricks secrets create-scope --scope inventory-stock-dev
```

Verify:
```bash
databricks secrets list-scopes | grep inventory-stock-dev
```
...
```bash
databricks secrets create-scope --scope inventory-stock-prod
```
...
```bash
databricks secrets put --scope inventory-stock-dev --key jdbc_url
databricks secrets put --scope inventory-stock-dev --key jdbc_username
...
```bash
databricks secrets put --scope inventory-stock-prod --key jdbc_url
...
dbutils.secrets.list(scope="inventory-stock-dev")
